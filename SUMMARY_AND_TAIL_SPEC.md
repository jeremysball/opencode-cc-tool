...
Add two observation tools for work delegated through `opencode_dispatch`:

- `opencode_tail` gives an immediate, bounded view of the newest model text.
- `opencode_summary` creates a short, semantic progress report from the task's observed narration.

The tools complement existing interfaces. `opencode_status` remains the source of lifecycle state, `opencode_wait` remains the blocking lifecycle primitive, and `opencode_result` remains the full completed-task result.

## Design Decisions

`opencode_tail` is deterministic and synchronous. It parses a bounded suffix of the task's existing NDJSON log, never starts a model, and returns the requested suffix of the latest `type: "text"` event.
...
`opencode_summary` is asynchronous. A semantic summary requires a model call, which may exceed an MCP client's request timeout. It runs under a dedicated, tool-denied OpenCode agent using `opencode-go/deepseek-v4-flash`, and immediately returns that task's identifier. The caller waits for and reads it using the existing `opencode_wait` and `opencode_result` tools.
...
| --- | --- | --- | --- |
| `task_id` | yes | | A task returned by `opencode_dispatch`. |
...
The `status` is captured with the log read. It may change immediately afterward, so it is advisory; callers requiring an authoritative terminal state use `opencode_wait` or `opencode_status`.
...
textTotalChars: 0
truncated: false
help[1]: Run opencode_wait with task_id "oc_..." to wait for task output
```
...
next: Run opencode_wait with task_id "oc_summary", then opencode_result with task_id "oc_summary"
```

The summary task uses the normal task manager lifecycle. It appears in `opencode_list`, can be waited on, and can be cancelled. `opencode_result` returns the report in its existing `message` field. If a server restart changes an active summary task to `unknown`, it is unavailable: `opencode_result` must return a definitive incomplete-state response rather than parse partial text.
...
help[1]: Run opencode_tail with task_id "oc_source" after the task emits output
```
...
`opencode_summary` sends captured task narration to the configured DeepSeek provider through OpenCode. Narration can contain repository details, command output, or secrets emitted by a delegated task. The tool description must disclose this transfer. Locally, the input exists only in a private temporary attachment until the summary child exits. `opencode_tail` is local and sends no task content to a model.
...
The summary agent must print only the report as its final message. `max_words` is a model target, not a hard output guarantee; the immediate result and final result label it as such. This keeps `opencode_result(...).message` directly usable without parsing agent narration.

## Selective Results

Extend `opencode_result` with an optional `fields` array. Omitted `fields` preserves the current full result shape. When present, the response contains only `taskId`, `status`, and the requested fields.
...
```text
opencode_result(task_id: "oc_...", fields: ["message"])
...
`fields` is a response projection, not a new parser. It cannot request partial text from a running task; use `opencode_tail` for live output.
...
- Make the summary model configurable through `OPENCODE_CC_TOOL_SUMMARY_MODEL`, defaulting to `opencode-go/deepseek-v4-flash`. Cache a capability preflight from `opencode models`; return an actionable error without creating a task when the configured model is unavailable.
- Add `fields` projection to `result(taskId, options)` and `opencode_result`. Keep its omitted-fields response backward compatible.
- Register both MCP tools in `src/server.js` with tight schemas and descriptions that disclose the summary provider.
- Do not alter `opencode_result` semantics. A summary task is an ordinary dispatched task whose final `message` happens to be a summary.
...
- `opencode_tail` returns the final complete text event without reading the entire log.
- `opencode_tail` returns an exact suffix and correct length/truncation fields.
- `opencode_tail` has a definitive no-text result and rejects invalid `chars` before log access.
- `opencode_summary` refuses to dispatch for an empty narration snapshot.
- `opencode_summary` sends the captured snapshot, requested word limit, and DeepSeek model ID to its child task.
...
- `opencode_result(fields: ["message"])` returns only its mandatory task envelope and final message; omitting `fields` preserves the current response.
...
- Replacing `opencode_wait`, `opencode_result`, or full narration retrieval.
